...
trait CapabilityProviderItem extends Item {
  ...
  def addCapability[T](cap: Capability[T], f: ItemStack => T): Unit = {
    val capability = new PartialFunction[ItemStack, T] {
      override def isDefinedAt(stack: ItemStack): Boolean = true
      override def apply(stack: ItemStack): T = f(stack)
    }
    caps += (cap -> capability.orElse(caps(cap)))
  }
