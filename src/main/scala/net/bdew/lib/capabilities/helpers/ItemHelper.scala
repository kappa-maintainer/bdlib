...
object ItemHelper {
  ...
  def getItemHandler(world: World, pos: BlockPos, side: EnumFacing): Option[IItemHandler] = {
    ...
    if (tile == null) None
      ...
      if (cap != null) Some(cap) else CapAdapters.get(CAP_ITEM_HANDLER).wrap(tile, side)
    } else CapAdapters.get(CAP_ITEM_HANDLER).wrap(tile, side)
  }
  ...
  def getItemHandler(stack: ItemStack): Option[IItemHandler] = {
    if (stack.isEmpty) None
      ...
      if (cap != null) Some(cap) else CapAdapters.get(CAP_ITEM_HANDLER).wrap(stack)
    } else CapAdapters.get(CAP_ITEM_HANDLER).wrap(stack)
  }

  def pushItems(from: IItemHandler, to: IItemHandler, maxItems: Int = Int.MaxValue, filter: (ItemStack) => Boolean = !_.isEmpty): Unit = scala.util.boundary {
    var itemsPushed = 0
    for {
      ...
      if (!canExtract.isEmpty) {
        ...
        if (itemsPushed >= maxItems) scala.util.boundary.break(())
      }
    }
  }
}
