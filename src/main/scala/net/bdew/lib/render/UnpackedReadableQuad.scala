...
class Unpacker(format: VertexFormat) extends IVertexConsumer {
  ...
  var tint = -1
  var orientation: EnumFacing = scala.compiletime.uninitialized
  var vertices = 0
  var elements = 0
  var full = false
  var texture: TextureAtlasSprite = scala.compiletime.uninitialized
  ...
  override def setQuadOrientation(orientation: EnumFacing): Unit = {
    this.orientation = orientation
  }

  def setTexture(texture: TextureAtlasSprite): Unit = {
    this.texture = texture
  }

  override def setApplyDiffuseLighting(diffuse: Boolean): Unit = {
