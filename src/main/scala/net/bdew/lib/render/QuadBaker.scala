...
class QuadBaker(format: VertexFormat) {
  ...
  def bakeMap[T](quads: Map[T, TQuad]): Map[T, BakedQuad] = quads.view.mapValues(bakeQuad).toMap
  def bakeListMap[T](quads: Map[T, List[TQuad]]): Map[T, List[BakedQuad]] = quads.view.mapValues(bakeList).toMap
  ...
  private def vertexToRaw(v: TVertex, face: EnumFacing, shading: Boolean): Array[Array[Float]] = {
    ...
    for ((element, index) <- format.getElements.asScala.zipWithIndex) {
...
object QuadBaker {
  private lazy val normals = EnumFacing.values().map(f => {
    f -> Array[Float](f.getXOffset.toFloat, f.getYOffset.toFloat, f.getZOffset.toFloat, 0)
  }).toMap
}
