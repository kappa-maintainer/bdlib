...
abstract class BaseTexturableModel extends IModel {
  ...
  override def getTextures = textures.values.map(x => new ResourceLocation(x)).toList.asJava
  override def retexture(textures: ImmutableMap[String, String]) = new RetexturedModel(this, textures.asScala.toMap)
