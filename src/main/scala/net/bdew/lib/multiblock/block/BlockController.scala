      ...
      for ((kind, max) <- machine.modules) yield {
        ...
        if (machine.required.getOrElse(kind, 0) > 0) {
          Misc.toLocalF("bdlib.multiblock.tip.module.range",
            TextFormatting.YELLOW.toString + name + TextFormatting.RESET.toString,
            TextFormatting.YELLOW.toString + machine.required(kind).toString + TextFormatting.RESET.toString,
            TextFormatting.YELLOW.toString + max.toString + TextFormatting.RESET.toString)
        } else {
