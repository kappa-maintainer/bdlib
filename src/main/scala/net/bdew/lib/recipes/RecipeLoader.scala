...
class RecipeLoader {
  ...
  def getAllConcreteStacks(s: StackRef, cnt: Int = 1): Iterable[ItemStack] = s match {
    case StackOreDict(id) =>
      ...
      val c = OreDictionary.getOres(id).asScala.map(_.copy()).toList
  ...
  /**
    ...
    */
  def processRecipeStatements(): Unit = {
  ...
  /**
    ...
    */
  def processRecipeStatementsSafe(list: List[RecipeStatement]): Unit = {
    for (s <- list) {
      try {
