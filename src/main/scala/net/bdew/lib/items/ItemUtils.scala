...
object ItemUtils {
  ...
  def dropItemToPlayer(world: World, player: EntityPlayer, stack: ItemStack): Unit = {
    if (stack.isEmpty || world.isRemote) return
  ...
  def addStackToSlots(stack: ItemStack, inv: IInventory, slots: Iterable[Int], checkValid: Boolean): ItemStack = scala.util.boundary {
    if (stack.isEmpty) scala.util.boundary.break(stack)
    ...
    for (slot <- slots if (!checkValid || inv.isItemValidForSlot(slot, stack)) && isSameItem(stack, inv.getStackInSlot(slot))) {
      ...
      if (toAdd >= stack.getCount) {
        ...
        stack.setCount(0)
        inv.markDirty()
        scala.util.boundary.break(ItemStack.EMPTY)
      } else if (toAdd > 0) {
        target.grow(toAdd)
        stack.shrink(toAdd)
        inv.markDirty()
  ...
  def addStackToHandler(stack: ItemStack, inv: IItemHandler, slots: Iterable[Int]): ItemStack = scala.util.boundary {
    if (stack.isEmpty) scala.util.boundary.break(stack)

    var left = stack.copy()
    for (slot <- slots if isSameItem(stack, inv.getStackInSlot(slot))) {
      left = inv.insertItem(slot, left, false)
      if (left.isEmpty) scala.util.boundary.break(left)
    }
