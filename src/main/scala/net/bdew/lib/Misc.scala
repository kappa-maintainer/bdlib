...
object Misc {
  ...
  def toLocalF(s: String, params: Any*) = I18n.translateToLocal(s).format(params*)
  ...
  def filterType[T](from: Iterable[?], cls: Class[T]) = from.filter(cls.isInstance).asInstanceOf[Iterable[T]]
  ...
  def getModVersionString(modId: String): String =
    modLookup.get(modId).map(cont => cont.getModId + " " + cont.getVersion).getOrElse("NOT FOUND")
