...
trait WidgetContainer extends DrawTarget {
  val rect: Rect
  val widgets = mutable.ListBuffer.empty[BaseWidget]
  ...
  def mouseClicked(p: Point, bt: Int): Unit = {
    for (w <- activeWidgets)
  ...
  def keyTyped(c: Char, i: Int): Boolean =
    activeWidgets.exists(_.keyTyped(c, i))

  def drawBackground(mouse: Point): Unit = {
  ...
  def draw(mouse: Point, partial: Float): Unit = {
    val p = mouse - rect.origin
    GL11.glPushMatrix()
    GL11.glTranslatef(rect.origin.x, rect.origin.y, 0)
    for (w <- activeWidgets) {
