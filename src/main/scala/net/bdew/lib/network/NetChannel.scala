...
class NetChannel(val name: String) {
  ...
  object ServerHandler extends SimpleChannelInboundHandler[Message] {
    def channelRead0(ctx: ChannelHandlerContext, msg: Message): Unit = {
      try {
  ...
  private def addHandler(side: Side, handler: ChannelHandler): Unit = {
    val ch = channels.get(side)
    val name = ch.findChannelHandlerNameForType(classOf[SerializedMessageCodec[this.type]])
    ch.pipeline().addAfter(name, side.toString + "Handler", handler)
  }
  ...
  def sendTo(message: Message, player: EntityPlayerMP): Unit = {
    channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.PLAYER)
    channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(player)
    channels.get(Side.SERVER).writeAndFlush(message).addListener(ChannelFutureListener.FIRE_EXCEPTION_ON_FAILURE)
  }

  def sendToAllAround(message: Message, point: NetworkRegistry.TargetPoint): Unit = {
